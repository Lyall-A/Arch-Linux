...
while true; do
    ...
    dump="$(pw-dump)"
    # Get all node ID's
    filtered_nodes_ids=$(echo "$dump" | jq -r '.[] | select(.type == "PipeWire:Interface:Node" and .info.props["media.class"] == "Stream/Output/Audio") .id')
    # Get default sink ID
    ...
    for id in $filtered_nodes_ids; do
        ...
        echo "$linked_to_sink $linked_to_other"
